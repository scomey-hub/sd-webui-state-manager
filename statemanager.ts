...
[]        but only when the resulting value is different from either [current UI value | current UI default]
    ...
    sm.buildComponentMap = async function(): Promise<void>{
        ...
        .then(response => {
            ...
            for(const accordion of inputAccordions){
                ...
                let checkbox: HTMLInputElement | null = checkboxContainer instanceof HTMLInputElement
                    ? checkboxContainer
                    : <HTMLInputElement>checkboxContainer?.querySelector('input');

                let visibleCheckboxContainer = accordion.parentElement!.querySelector(`#${accordion.id}-visible-checkbox`);
                let visibleCheckbox: HTMLInputElement | null = visibleCheckboxContainer instanceof HTMLInputElement
                    ? visibleCheckboxContainer
                    : <HTMLInputElement>visibleCheckboxContainer?.querySelector('input');
    ...
    sm.getComponentValue = function(mappedComponent: MappedComponent): any {
        ...
        if (element) {
            ...
            const dropdownValue = element.querySelector('input[type="hidden"]') as HTMLInputElement;
            if (dropdownValue) {
    ...
    sm.setComponentValue = function(mappedComponent: MappedComponent, value: any): void {
        const element = mappedComponent.element;
        const component = mappedComponent.component;

        // Try setting on DOM element first (Gradio 4.x compatible)
        if (element) {
            let targetInput: HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement | null = null;
